...
#' @export
getbiotechrep <- function(xset, method = "medret", intensity = "into", file = NULL, rsdcf = 30, inscf = 5) {
        ...
        message(paste(N,'out of',L,'peaks found with rsd cutoff',rsdcf,'and Log intensity cutoff', inscf))
...
#' @export
getbiorep <- function(xset, method = "medret", intensity = "into", file = NULL, rsdcf = 30, inscf = 5) {
        data0 <- xcms::groupval(xset, method, intensity)
        data0[is.na(data0)] = 0
        data <- t(data0)
        lv <- xset@phenoData[, 1]
        ...
        indexrsd <- apply(t(rsd[,-1]), 1, function(x) any(x < rsdcf))
        indexmean <- apply(t(mean[,-1]), 1, function(x) any(x > 10^(inscf)))
...
plotmr <- function(xset,
        ...
        suppressWarnings(if(!is.na(data)){
                ...
                legend('top', legend = dataname, col = col, pch = 19, horiz = T, bty = 'n')
        }else{
                graphics::plot(
                        1,
                        xlab = "Retention Time(s)",
...
plotmrc <- function(xset,
        ...
        suppressWarnings(if(!is.na(data)){
                ...
                graphics::legend('top', legend = c(name1,name2),pch = 19,col = c(grDevices::rgb(0,0, 1, 0.618),grDevices::rgb(1,0, 0, 0.618)),bty = 'n',horiz = T)}else{
                        graphics::plot(
                                1,
                                xlab = "Retention Time(s)",
                                ylab = "m/z",
